...
class aggregation_LP(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.conv5          = nn.Conv2d(channel, 3, 1) #ESTA ES LA QUE DEBERIA CAMBIAR ¿? #CAMBIEN EL 1 POR EL 3
...
class aggregation_LP3(nn.Module): #ESTA ES PARA VER LA ACTIVACION DE GAMMA
    # dense aggregation, it can be replaced by other aggregation previous, such as DSS, amulet, and so on.
    # used after MSF
    def __init__(self, channel):
        super(aggregation_LP3, self).__init__()
        channel = channel * 4
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv_upsample1 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv4 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv5 = nn.Conv2d(channel, 1, 1)  # Cambiado de 3 a 1 canal
...
class PraNet(nn.Module):
    ...
    def __init__(self, channel=32, use_attention='PCM', mode_cls = 'max_pooling'):
        ...
        self.agg_LP3 = aggregation_LP3(channel)
    ...
    def forward(self, x):
        ...
        if self.use_attention == 'PCM':
            ...
            pcm = self.agg_LP(pcm_t) #PRINTEAR ACA
            pcm2 = self.agg_LP3(pcm_t) #NO USAR ESTA
            pcm_ = F.interpolate(pcm, scale_factor=4, mode='bilinear')   # NOTES: Sup-1 (bs, 1, 44, 44) -> (bs, 1, 352, 352)
            pcm_2 = F.interpolate(pcm2, scale_factor=4, mode='bilinear')#Aqui guardo la activacion y la subo al tamaño de la salida de la conv 1x1x3
            pcm_tprom = F.interpolate(pcm_t, scale_factor=8, mode='bilinear')
            print(pcm_2.shape, pcm.shape, pcm_tprom.shape)
            ...
            # segmentation = torch.round(torch.sigmoid(lateral_map_5)) AQUI
            #classifcation = max_pool(torch.sigmoid(pcm_))     
        
        return pcm_, pcm_2 , pcm_tprom #, classifcation
