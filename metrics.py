...
def calculate_mae(seg, image):
    seg_normalized = (seg - seg.min()) / (seg.max() - seg.min())
    image_normalized = (image - image.min()) / (image.max() - image.min())    
    mae_loss = F.l1_loss(seg_normalized, image_normalized)   
    mae_loss = mae_loss * 100
    mae_loss_value = mae_loss.item()

    return mae_loss_value

def metrics_mae(preds, label, dataset): 
  tp, fp, tn, fn = 0, 0, 0, 0
  TP, FP, TN, FN = [], [], [], []
  umbral = 13.6621  #AQUI SE CAMBIA EL UMBRAL  14.9918 es para el balanceado y 13.6621 Para tener mejor recall!! 7.5913  balanceado lr4 y mejor recall 7.0628  mejor prec 8.1198 mejor prec lr6 16.3215 
  for index, pred in enumerate(preds):
    if pred >= umbral and label[index] == 1:  
        tp = 1
        TP.append(tp)
    if pred >= umbral and label[index] == 0:  
        fp = 1
        FP.append(fp)
    if pred < umbral and label[index] == 1:   
        fn = 1
