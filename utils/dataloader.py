...
class PolypData(data.Dataset):
    def __init__(self, image_root, trainsize): #MODIFIQUE ACA
        ...
        #self.gt_transform = transforms.Compose([
         #   transforms.Resize((self.trainsize, self.trainsize)), MODIFUE AQUI
          #  transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        name_img = self.images_root[index].split('/')[-1][0]
        name_img2 = self.images_root[index].split('/')[-1]
        #print(name_img)
        #print(image.shape, image.max())

        #gt = self.binary_loader(self.gts_root[index]) MODIFIQUE AQUI
         ...
         #   name = 1
          #  name_img = self.images_root[index].split('/')[-1]
           # index_image = np.where(int(name_img[:-4]) == self.polyp_image)[0][0]
            ...
            #label = int(name)
        #else: 
         #   name = self.images_root[index].split('-')[0]   MODIFIQUE AQUI
          #  name_img = self.images_root[index].split('/')[-1]
           # label = int(name[-1])
            #label_histhology = 0
        ...
        return image, name_img, name_img2 #MODIFUQE AQUI
    ...
    #def filter_files(self): ESTA FUNCION HAY QU QUITARLA, poruqe  en train solo se va a usar fondo
...
def get_loader(image_root, batchsize, trainsize, shuffle=True, num_workers=4, pin_memory=True): #AQUI MODIFIQUE
    #path = '/home/linamruiz/Polyps/tesismaestria/LPSegNet/dataset_cls/data/m_train/train_2.csv'
    dataset = PolypData(image_root, trainsize) #AQUI
